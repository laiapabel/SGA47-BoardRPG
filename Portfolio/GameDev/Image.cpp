...
void Image::Draw(HDC hdc)
{
   ...
   if (bUseRotate)
   {
      ::GdiTransparentBlt(hdc,							// 그려질 DC
         ...
         rcDestTemp.right, rcDestTemp.bottom,	// 그려질 크기
         hBitmapDC,									// 이미지 DC
         rcSrcTemp.left, rcSrcTemp.top,				// 이미지 부분 위치
         rcSrcTemp.right, rcSrcTemp.bottom,
   ...
   else if (bUseTransparent)
   {
      ::GdiTransparentBlt(hdc,									// 그려질 DC
         ...
         rcSrc.right, rcSrc.bottom,		// 그려질 크기
         hBitmapDC,								// 이미지 DC
         rcSrc.left, rcSrc.top,			// 이미지 부분 위치
         rcSrc.right, rcSrc.bottom,	// 이미지 부분 크기
...
void Image::SetSize(const Rect& rc)
{
	rcSrc = rc;
}
